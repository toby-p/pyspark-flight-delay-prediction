...
# MAGIC %md
# MAGIC # Full Data Pipeline
# MAGIC ## Team 8
# MAGIC 
# MAGIC Data pipeline includes the following steps:
# MAGIC 1. Data Transformation (Joining Weather and Flight dataset)
# MAGIC   - Reduce the size of weather dataset by keeping only the rows relevant to airports on recordes
# MAGIC   - Use external dataset to get weather station ID (IATA code)
# MAGIC   - Aggregate flight data with weather data
# MAGIC 2. Feature engineering
# MAGIC 3. Fill missing values and split the data into tran/test set for ML pipeline and save them into blob storage
